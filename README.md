...
**And a fork can be played.** A copy is watched and never played at — what happened at it has already happened — but a fork is no longer that game, so it is a table of its own and takes the game's own verbs. Its seats stop being watching keys and become seats: the panes on one hold them and act as the players. It is stopped when it is made, because forking is setting a game up rather than starting one, and **Carry on** starts it — the panes can bid and play, and the bots take their turns.

That control has a cluster of its own on the band, marked **Fork**, and says which way it is: *stopped* or *playing*, in gold. It is not in the transport, and it must not be: the transport plays a *tape* back and this plays the *game*, and the two were side by side, both green and both starting with ▶ — two clocks wearing one face. Press the wrong one and the table sits there while the tape runs, the phones say ❚❚ paused, and nothing else on the page agrees. The head line says it too — *table 4F2K, changed by hand · stopped · point 7 of 8*.
