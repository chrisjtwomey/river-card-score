...
- **Card** — a panel under the band, and the heart of the page: the scorecard in the shape the game already draws it, a row a round and a column a seat, the round in play marked and the rounds already scored tinted. It is both the way about the game and the way into it. Press a round's label and the game is taken there — a forced phase and round, nothing invented, so it works on any server; the **bid / tricks** toggle beside it says where it lands. Type in a cell and that round is edited where it stands, whatever round the game is on: putting a mistyped trick right three rounds back does not mean moving the game to get at it. Each cell is the pair the scorecard shows, what was bid and what was won. A bid goes on its own, because the table keeps a bid row with gaps in it; a won row goes whole, once every seat in it has a number, and the cells still wanted are ringed until it does. The totals under it follow every edit.
- **Fill to here** — beside the card, on a dev server only. It rebuilds the card and plays every round before the one the game is on, with bids and tricks a real table could have made. That is the destructive one, so it is named, kept away from the labels that only move the game, and asks first.
...
- **Players** — a panel under the band, one row a seat, and only what belongs to the seat rather than to a round: name, who hosts, who deals, bot, left, a photo on or off. Everything lands as it is changed. What a seat bid and won belongs to the round it happened in, so it is on the card.
...
What the page may do follows the **server**, not the table. On a server started with `DEV=1` a real table takes every control a table of stand-ins does — jump to, fill scorecard, randomise, all of it. The top bar turns red and the page says real players may be at the table, because every click lands on their game; nothing is taken away. On a normal server the host token opens the three controls that put a game right and invent nothing: the **Card**, the **Players** panel, and the **State** record itself. Everything that makes data up is refused — and, being refused, is not shown. The page is told which kind of server it reached in the same breath as the table, so the tables strip, **New table**, **Fill to here** and the one-shots are put away before anything is drawn. A control that draws itself and then answers a refusal teaches the limits one click at a time; this way the page offers what works and nothing else.

The card is the same card on either kind of server, because every edit on it is a forced value and forced values are what the host token has always been allowed. That is what a game stuck in the wrong phase needs — bidding that will not close, a finish that came too early, a round that has to be gone back over — and what a mistyped trick three rounds back needs, which is the same card, three rows up. What changes with the server is only whether the card can also be **filled**.
