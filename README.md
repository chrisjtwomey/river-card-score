...
5. **The deal.** Every round opens with the deal on every screen: the deck shuffled and dealt round the table, with a line over the deck naming whose deal it is — *Ann is shuffling…* where the phones are dealt to, *Ann is dealing…* where the cards are real. On the TV screen it stays up while the bids come in, each bid stamped onto that player's pile. On a phone with real cards only the shuffle plays, and the scene clears itself before any card goes out — a tap skips it.
