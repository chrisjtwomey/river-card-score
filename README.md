...
In the scrubber's place stands **Put to** — a phase, a round, and one button. It forces the game to that phase and that round and does nothing else: no rounds are built, no bids are made up, nothing is played. That is the whole of what the protocol takes on a normal server, and it is what a game stuck in the wrong phase needs — bidding that will not close, a finish that came too early, a round that has to be gone back over. The form says where the game is until you aim it somewhere else, and once aimed it stays put while the table moves, so a tap on **Put** changes only what you changed. A round travels only with a phase that has one: the lobby is put to on its own, and the finish picks its own round.
