...
Two levels of moving about in it, because a game is two levels: a mark a round in the strip — a hand thrown in gets its own, because it was a second go and looked different — and under it the **Points** bar, the round on show as a run from one end to the other with a bubble on it a point: a bid, a card, a trick taken, the score. The bar fills to the bubble it is standing on. The bubbles are not a slider, because nothing here is continuous; each of those either happened or has not, and each is a place to go.

Pass over a bubble and it says what happened there — *Nia plays 9♠*, *Amy bids 3* — on the line beside the bar, and as the browser's own tip under the pointer. That sentence is made once, on the server, when the copy is made: the line under the band says the same one for the point the copy is standing on, so the two never drift apart. Leaving the bubble puts the table's own line back. **⌂ Stop watching** lets the copy go. While one is open the panes are the copy's, and none of them can be acted in: the game they show has already been played.
