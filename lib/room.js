...
module.exports = ({ G, A, token, saveGame, DEV }) => {
  ...
  function countPublic(room) {
    ...
    return { turn: null, trick: [], won: p.won, last: p.last, upcard: null,
             counts: room.seats.map(() => left), log: p.log };
  }

  // The bids are in: with the cards on the phones the player left of the
  // dealer leads; with real cards the table plays and counts as it goes.
  ...
  return { curRound, seatIndex, Deck, create, seat, syncCfg, addBot, openRound, startGame, toLobby,
           seatBid, seatReady, closeBidding, countTrick, uncountTrick, bumDeal, scoreRound, finishGame, unfinish, undo,
