...
module.exports = ({ G, A, token, saveGame, DEV, Trail }) => {
  ...
  /* What the clocks on a table say now. The server owns the time and the
     ...
     `ms` is how long a seat may be idle, and how long before that it is
     warned. Either at nought turns that clock off.
     ...
     Returns what happened, for the server to tell: the seats to warn, and the
     seats that went and how. */
  function sweep(room, now, ms) {
    const out = { warn: [], gone: [], changed: false };
    ...
    if (!ms.idle) return out;
    const warnAt = ms.idle - (ms.warn || 0);
    room.seats.slice().forEach((s) => {
      ...
      if (gone >= ms.idle) {
        if (room.phase === 'lobby') { kickSeat(room, s.id); out.gone.push({ seat: s, how: 'kicked' }); }
        else if (G.virtual(room)) { standDown(room, p); out.gone.push({ seat: s, how: 'left' }); }
        // With real cards the table holds no hand of theirs, so it takes
        // nothing: it stops on that seat and waits on whoever runs it.
        else if (!room.stalled) room.stalled = { id: s.id, ms: ms.idle };
        else return;
        out.changed = true;
  ...
  return { curRound, seatIndex, Deck, BID_HOLD, create, seat, syncCfg, become, addBot, openRound, startGame, toLobby,
           kickSeat, standDown, idleSeat, sweep, carryOn,
