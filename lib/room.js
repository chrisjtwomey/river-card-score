...
module.exports = ({ G, A, token, saveGame, DEV, Trail }) => {
  ...
  function create(code, hostToken) {
    return {
      ...
      captainId: null,            // the player who runs the table from their device
      seats: [],
      rounds: [],
      idx: 0,
      ...
      stalled: null,              // the seat the table is waiting on, for the host to answer
  ...
  /* The round in play opens for its bids. This is the one place a round is
     ...
     devices this is also where they are dealt.
  ...
  function startGame(room) {
    ...
    room.idx = 0;
    room.paused = false;         // a new game is not somebody else's pause
  ...
  /* One bid, however it arrived: from a device, from the host for an empty
     seat, or from a bot. The last bid closes the bidding, so this is the one
     place that decides it. */
  /* A device says the round it was dealt is on its screen: the deal played out,
  ...
  /* With real cards the table counts the tricks as they are taken: one tap a
     trick, on any device or on the TV screen, and the last one scores the
  ...
  /* The bids are in -- but the table does not play off the last one. The
     round goes to tricks with nobody on play, so no device and no bot can put
  ...
  /* The bids have been read. With the cards on the devices the player left of
  ...
  /* The seat goes, and with it every trace of that player: this is only ever
     ...
     Returns the seat that went, so whoever asked can tell its device. */
  /* A seat off the table altogether. Only in the lobby: once a game has
     ...
     three doors reach this: a device leaving, the table host putting a seat
  ...
  /* A player put out of a game already in play, by whoever runs the table.
     Somebody who has to stop and cannot press it themselves, or somebody the
     table wants rid of.

     The seat cannot go -- the scorecard is a column for it and the rounds
     already played are that player's -- so it stays and the table takes it,
     which is `standDown`. What is different is the key: it stops working, so
     the device that held the seat cannot simply come back to it. That is the
     whole of what makes this different from a seat that went quiet, and the
     reason it is a verb of its own rather than a flag on that one.

     `letBack` is the way in again, and it is the host's alone: it opens the
     seat, and whoever is let in comes to it by the name it plays under. */
  function removeSeat(room, id) {
    const p = seatIndex(room, id);
    const seat = room.seats[p];
    if (!seat || seat.bot) return null;
    if (room.phase === 'lobby') return null;    // there the seat itself goes: kickSeat
    seat.token = null;
    standDown(room, p);
    T.point(room, { k: 'F', f: T.frame(room) });
    return seat;
  ...
  /* A seat the table was given, handed back. The player it belongs to is not
     ...
     seat is opened and they come back to it by name, exactly as a device that
  ...
  /* The table is standing on a beat that nothing is left to end: a finished
     ...
     for ever with every device waiting on it.
  ...
  /* Who deals. Before a game it is the seat that deals round one; during the
     ...
     so a table that deals on the devices throws the hand in instead. Which
  ...
  /* Whether this seat's idle clock is running. A device that is open with
     nothing to do is not idle, whatever it has not done: the clock runs while
     nobody holds the seat, and while the table is waiting on it and can go no
  ...
  /* One seat's answer to a bum-deal vote. One no ends it -- a hand is thrown
     ...
     Its own verb because two doors reach it: a device answering the vote, and
  ...
  /* A line in the talk, from a seat or from the table itself. The trimming,
     ...
     device's own chat, and the dev page saying something as somebody. `keep` is
  ...
  function sweep(room, now, ms) {
    ...
    if (ms.idle) room.seats.slice().forEach((s) => {
      ...
      if (gone >= ms.idle) {
        ...
        // Only a device that is here can be asked, and only once.
        s.warned = true;
        out.warn.push(p);
      }
    });
    ...
    return {
      ...
      seats: room.seats.map((s, p) => ({ id: s.id, name: s.name, online: s.online,
                                        ...
                                        /* How long this seat's clock has been running, or 0
                                           ...
                                           clocks to agree, and a device's does not. */
      ...
      // A TV screen that runs this table is at it. The table host's device
  ...
  return { curRound, seatIndex, Deck, BID_HOLD, create, seat, syncCfg, become, addBot, openRound, startGame, toLobby,
           ...
           seatBid, seatVote, say, seatReady, closeBidding, openPlay, countTrick, uncountTrick, bumDeal, scoreRound, finishGame, unfinish, setPaused, removeSeat,
