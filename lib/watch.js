...
module.exports = ({ createRoom, roomOf, listGames, send, fail, broadcast,
  ...
  function handleWatch(ws, m, room) {
    ...
    /* A fork carried on, or paused. A copy is paused the moment it is made,
    ...
    /* Branching on purpose, rather than as a side effect of changing
       something. The copy becomes a table of its own from here: the tape after
       this point goes, because it describes a game that is no longer going to
       be played. Pressed again from further back on a fork's own tape, it
       drops that tail the same way -- and `from` is kept as it was, so Reset
       still goes back to where the copy left the game. */
    else if (m.do === 'fork') return forked(ws, copy);
