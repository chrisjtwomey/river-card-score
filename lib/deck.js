...
/* The virtual deck: the server is the dealer.
   ...
   are held here too, so a device cannot renege, play out of turn, or play a
...
*/
module.exports = ({ G, curRound, scoreRound, T }) => {
  /* How long a finished trick stays up before the winner leads. It is longer
     than the devices take to name who took it and gather it in (TOOK_HOLD in
  ...
  function dealHands(room) {
    ...
    room.play = { round: room.idx, hands, upcard: up, trick: [], turn: null,
                  ...
                  // out, and which devices have said their table is up.
  ...
  function playPublic(room) {
    const p = room.play;
    if (!p) return null;
    return { turn: p.turn, held: !!p.held, trick: p.trick, won: p.won, last: p.last,
  ...
  function putCard(room, p, card) {
    ...
    // Nothing down yet, so this card opens a trick: the one moment that is true
    // of every trick, the first of a round included.
    if (!play.trick.length) { play.last = null; T.point(room, { k: 's' }); }
    const hand = play.hands[p];
    hand.splice(hand.indexOf(card), 1);
    play.trick.push({ p, card });
    T.point(room, { k: 'c', p, x: card });
    if (play.trick.length < n) { play.turn = (p + 1) % n; return null; }
    const winner = G.trickWinner(play.trick, r.trump);
    // The trick is taken here and nowhere else: settleTrick runs a beat later
    // and may never run at all, where the game has moved on over the top of it.
    T.point(room, { k: 'w', p: winner });
    play.won[winner] += 1;
