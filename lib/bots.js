...
/* Players the table provides, for a hand short of people.
   ...
   from a device. There is no cheating available to it -- not because it is
...
module.exports = ({ G, curRound, broadcast, seatBid, playCard, bumDeal }) => {
  ...
  /* A round is dealt on the devices before it is bid: the deck is shuffled, the
     ...
     So the first hand a bot bids in a round waits for the devices. Each says
     ...
     long the scene runs. A device that says nothing is waited on only so long. */
  ...
  /* A seat the table plays. A bot is one from the moment it sits down. A
     ...
     plays it out rather than waiting for a device that is not coming back.
  ...
  const anyAuto = (room) => G.tablePlaysOn(room) && G.tableSelfPlays(room);

  // Whether every device that is being dealt to has its table up. Nothing to
  ...
  /* One move, now, whatever a pause says: the dev page walking a hand through
     a step at a time. It is the same move the driver would have made when the
     timer came round -- nothing here knows anything the bots do not. */
  function step(room) {
    stop(room);
    if (!G.tableSelfPlays(room)) return;
    act(room);
  }
  ...
  function act(room) {
    ...
    if (room.phase === 'tricks' && room.play && G.virtual(room)) {
      ...
      // Through the same door a device's card goes through: the rules are the
      // deck's, and a bot gets no say in them.
      playCard(null, room, p, card);
    }
  }

  return { NAMES, DELAY, DEAL_WAIT, bidFor, cardFor, botName, nudge, stop, step, anyAuto, dealt };
};
