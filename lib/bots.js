...
*/
module.exports = ({ G, curRound, broadcast, virtual, seatBid, playCard, bumDeal }) => {
  ...
  const DELAY = Math.max(120, Number(process.env.BOT_DELAY) || 850);
  ...
  // Whose turn it is, whether that seat is a bot or a person.
  function turnOf(room) {
    const r = curRound(room);
    if (!r) return null;
    if (room.phase === 'bid') return G.turnSeat(r, room.seats.length);
    if (room.phase === 'tricks' && room.play) return room.play.turn;
    return null;
  }

  const anyBots = (room) => room.seats.some((s) => s.bot);
  ...
  function voteOwed(room) {
    ...
    return room.seats.some((s, i) => s.bot && !v.yes.includes(i) && !v.no.includes(i));
  }
  ...
  function nudge(room) {
    ...
    if (!anyBots(room)) return;
    const p = turnOf(room);
    const onPlay = p !== null && p !== undefined && room.seats[p] && room.seats[p].bot;
  ...
  function act(room) {
    if (voteOwed(room)) {
      ...
      room.seats.forEach((s, i) => {
        if (s.bot && !v.yes.includes(i) && !v.no.includes(i)) v.yes.push(i);
      });
    ...
    const p = turnOf(room);
    if (p === null || p === undefined || !room.seats[p] || !room.seats[p].bot) return;
    const r = curRound(room);
    if (!r) return;
    ...
    if (room.phase === 'tricks' && room.play && virtual(room)) {
  ...
  return { NAMES, DELAY, bidFor, cardFor, botName, nudge, stop, turnOf, anyBots };
};
