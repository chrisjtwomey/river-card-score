...
module.exports = ({ G, curRound, broadcast, virtual, seatBid, playCard, bumDeal }) => {
  ...
  /* A seat the table plays. A bot is one from the moment it sits down. A
     ...
     A left seat at a table with real cards has no hand to read, so the table
     cannot play it: that one waits for the host. */
  const auto = (room, s) => !!s && (s.bot || (!!s.left && virtual(room)));
  const anyAuto = (room) => room.seats.some((s) => auto(room, s));
  ...
  return { NAMES, DELAY, bidFor, cardFor, botName, nudge, stop, turnOf, anyAuto };
};
