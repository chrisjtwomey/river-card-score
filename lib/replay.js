...
module.exports = ({ Room, G, token, Trail }) => {
  ...
  function apply(sandbox, ev) {
    ...
    switch (ev.k) {
      ...
      /* With cards on the devices the card took the trick as it landed, so this
  ...
  /* A copy is watched, never played, so its seats get a watching key and never
     ...
     A copy that has been changed is a table of its own, and its seats are held
     rather than watched: it gets a key a device can act with, so the panes on a
     fork can play the hand you set up. Stand-ins, because nobody real is
     behind them -- which is also what keeps the idle clock off it. Stopped
     either way: forking is setting a game up, not starting one, and the table
     is carried on by hand when it is ready. */
  function mintKeys(sandbox) {
    const own = !!(sandbox.replay && sandbox.replay.forked);
    sandbox.seats.forEach((s) => {
      s.watch = s.watch || token();
      if (own) s.token = s.token || token();
      else { s.token = null; s.online = false; }
    });
    sandbox.stand = own;
    sandbox.paused = true;       // nothing here plays itself until it is told to
  }
  ...
  function seek(sandbox, at) {
    ...
    /* Putting the trail back runs the game's own verbs, and those write down
       what they do. A copy being read has nothing to write: this says so, so
       that seeking a fork does not append every point it walks through. */
    r.reading = true;
    try { reseek(sandbox, at); } finally { r.reading = false; }
  }

  function reseek(sandbox, at) {
  ...
  function fork(sandbox) {
    const r = sandbox.replay;
    if (!r) return -1;
    /* Where this copy left the game it is a copy of, and what that game was,
       kept once. Everything from the first change on is the copy's own, so
       this is the whole of the way back -- and `fork` never writes into the
       array it was given, it makes a new one, so the game's own tape is still
       exactly what it was. */
    if (!r.from) r.from = { at: Math.max(0, r.at), points: r.points };
    ...
    mintKeys(sandbox);        // its seats are held now, not watched
    return r.at;
  }

  /* A fork put back. Everything the copy became goes -- the change, and
     whatever was played on it after -- and what is left is the game it is a
     copy of, standing at the point it was changed at. Its seats go back to
     being watched rather than held, and it stops, because `mintKeys` reads
     `forked` and it no longer is one.

     Nothing is lost that was not this copy's: the game on file was never
     touched, and the tape being put back here is the one it was opened with. */
  function unfork(sandbox) {
    const r = sandbox.replay;
    if (!r || !r.from) return false;
    const back = r.from.at;
  ...
  function open(sandbox, of, points) {
    sandbox.replay = { of, at: 0, playing: false, rate: 1, points };
    seek(sandbox, 0);
    return sandbox;
  }
  ...
  function say(sandbox) {
    const r = sandbox.replay;
    return {
      t: 'replay', code: sandbox.code, of: r.of, at: r.at, n: r.points.length,
      playing: !!r.playing,
      rate: r.rate || 1,
      ...
      /* A copy is watched, so its seats are handed over as watching keys. A
         fork is played, so its seats are handed over as seats: the panes on
         one act as the players, which is the whole of what a fork is for. Only
         the socket that opened the copy is ever told, and nothing else can
         reach it -- a copy is not a table anybody joins by code. */
      seats: sandbox.seats.map((s) => Object.assign(
        { id: s.id, name: s.name, watch: s.watch }, r.forked ? { token: s.token } : null)),
      ...
      /* And every one of them in words, for the timeline to say on the way
         past, and what each has to show for itself. Worked out here rather
         than kept: a fork lengthens its own tape as it is played, and a list
         made once would stop describing it at the first card. */
      says: lines(r.points),
      faces: r.points.map(faceOf),
      game: r.game || null,
      where: where(sandbox),
    };
  }
  ...
  const at = (sandbox) => ({
    t: 'replayAt', code: sandbox.code, at: sandbox.replay.at, n: sandbox.replay.points.length,
  ...
  return { open, seek, step, rate, fork, unfork, say, at, marks, ready, did, lines };
};
