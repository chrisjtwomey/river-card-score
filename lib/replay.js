...
module.exports = ({ Room, G, token }) => {
  ...
  function say(sandbox) {
    ...
    return {
      ...
      // And every one of them in words, for the stepper to say on the way past.
      says: r.says || [],
      game: r.game || null,
      where: where(sandbox),
    };
  ...
  const at = (sandbox) => ({
    ...
    playing: !!sandbox.replay.playing, where: where(sandbox),
    state: Room.publicState(sandbox),
  });
  ...
  function did(ev, who) {
    switch (ev.k) {
      ...
      case 'F': return 'the table was forced';
  ...
  return { open, seek, step, say, at, marks, ready, did, lines };
};
