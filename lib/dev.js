...
module.exports = ({ DEV, G, createRoom, roomOf, listTables, endTable, attach, send, fail,
                   broadcast, setAvatar, Room, Tables, Bots }) => {
  ...
  /* The whole table, replaced by an edited copy of its own record -- the
     ...
     The keys are the table's own, never the text's: the host token stays, and
     each seat keeps the token and the watch token it had. A record read off a
     real table carries none of them, so a paste that has been round-tripped
     must not leave the table unopenable -- and one that invents them must not
     hand anybody a way in. Where the record came back with no hands, the hands
     in play stay as they were. */
  function devState(room, rec) {
    ...
    const keep = ['code', 'hostToken', 'sockets', 'seen', 'botTimer', 'bidTimer'];
