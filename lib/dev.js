...
/* The dev controls, and nothing a game needs.
   ...
   which is nothing to do with this file: it needs no table and no key, and
   `{ t: 'replay' }` is a message of its own (lib/watch.js).
...
module.exports = ({ DEV, G, createRoom, roomOf, listTables, endTable, attach, send, fail,
  ...
  function handleDev(ws, m) {
    ...
    /* What may be done follows the server. A dev server gives every table
       ...
       A replay is not here at all: it is a message of its own, because it
       invents nothing and needs no table. See lib/watch.js. */
