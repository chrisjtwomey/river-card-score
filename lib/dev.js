...
module.exports = ({ DEV, G, createRoom, roomOf, listTables, endTable, attach, send, fail,
  ...
  /* One thing done to one seat, from the outside. Every one of them is a room
     ...
     `leave` is the phone's own Leave, which no host-side message can say --
     `{ t: 'leave' }` is the player's. `kick` and `back` are the table's own
     messages, said here so the panel has them all in one row. `out` is the
     idle clock run out on that seat, which the page could not reach at all: a
     table of stand-ins is never idle, so the clock never fires on one. */
