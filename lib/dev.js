...
/* The dev controls, and nothing a game needs.
   ...
   real game can reach.
*/
module.exports = ({ DEV, G, createRoom, roomOf, listTables, endTable, attach, send, fail,
                   broadcast, setAvatar, Room, Tables, Bots, Trail, Replay, dropRoom }) => {
  ...
  function devReplay(ws, room, m) {
    ...
    if (m.do === 'close') { shut(); return send(ws, { t: 'replay', code: null, of: room.code }); }

    if (m.do === 'open') {
      const points = Trail.read(room.code);
      if (!points.length) return fail(ws, 'nothing has been written down about this table yet');
      shut();
      const copy = createRoom();
      Replay.open(copy, room.code, points);
      ctx.replay = copy.code;
      broadcast(copy);
      return send(ws, Replay.say(copy));
    }

    const copy = ctx.replay && roomOf(ctx.replay);
    if (!copy || !copy.replay) return fail(ws, 'open a replay first');
    if (m.do === 'seek') Replay.seek(copy, Number(m.at));
    else if (m.do === 'step') Replay.step(copy, Number(m.by) || 1);
  ...
  function handleDev(ws, m) {
    ...
    /* What may be done follows the server. A dev server gives every table
       ...
       A replay is in that company because it invents nothing either: it puts
       back what already happened, on a copy, from that table's own record. The
       table it is of is not touched, which is the whole reason for the copy. */
    if (!['patch', 'state', 'replay'].includes(m.action) && !DEV) {
