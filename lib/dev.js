...
module.exports = ({ DEV, CHAT_KEEP, G, createRoom, roomOf, listTables, endTable, attach, send, fail,
  ...
  // device the table itself calls its host.
  ...
  /* The cards in every hand. They are never in the state a screen is sent --
     a hand is a secret, and the server sends each socket its own cards only --
     so reading them all at once is a door of its own, and a dev server's
     alone. The players panel asks for them to draw the picker, and again
     whenever the table moves under an open one: what it draws has to be what
     is in the hand now, not what was in it when the panel opened. */
  function devHands(ws, room, replay) {
    if (!DEV) return fail(ws, 'reading every hand needs the server started with DEV=1');
    const play = room.play;
    return send(ws, Object.assign({ t: 'handsRaw',
      hands: (play && play.hands) ? play.hands.map((h) => h.slice()) : null },
      replay ? { replay: true } : null));
  }
  ...
  function devPatch(room, p) {
    ...
    /* The hands, dealt by hand. A deck is 52 cards, no card is in two places,
       and a round of five is five cards a hand -- so what comes in is held to
       all three: anything that is not a card is dropped, a card already given
       to an earlier seat is dropped from the later one, and a hand stops at
       the round's own size. None of those is a state a real game reaches, and
    ...
    if (p.hands && room.play && DEV && Array.isArray(p.hands)) {
      ...
      const most = round ? round.cards : 13;
      room.play.hands = room.seats.map((x, i) => {
        const want = Array.isArray(p.hands[i]) ? p.hands[i] : [];
        const out = [];
        want.forEach((c) => {
          const card = String(c);
          if (!real.has(card) || given.has(card) || out.length >= most) return;
          given.add(card);
          out.push(card);
        });
        return out;
  ...
  /* One thing done to one seat, from the outside. Every one of them is a room
     ...
     Every one of them comes through this door and not through the table's own
     messages, because the page's socket is not always at the table it is
     driving: watching a game again, it is at no table at all, and the copy is
     reached by name. So a seat verb has to be sayable from outside. The rules
     they lean on are the room's -- `kickSeat` will not take a seat off a table
     that has started, whoever asks -- so nothing is agreed twice. */
  function devSeatDo(room, m) {
    ...
    switch (m.do) {
      case 'leave':
        ...
        return null;
      case 'kick':
        if (!Room.kickSeat(room, s.id)) return 'a seat only leaves the table in the lobby';
        return null;
      case 'back':
        if (!Room.letBack(room, s.id)) return 'there is no seat there for the table to give back';
        return null;
      case 'out':
        if (!Room.giveUp(room, p)) return 'the table is already waiting on a seat';
        return null;
      /* This seat's answer to a bum-deal vote, which no host-side message can
         say either: a vote is answered by the device it is put to. */
      case 'yes': case 'no':
  ...
  // only for a table of stand-ins, so the previews can open each device. A real
  // table never hands its seats out.
  function devHello(ws, room) {
    // A table of stand-ins hands over the seats themselves, so every device in
  ...
  function handleDev(ws, m) {
    ...
    /* The copy, read and written. A replay is what the trail says happened
       ...
       Only the forcing controls get here, which is the whole of what the page
       offers on a copy: the record, the players panel, and the cards in the
       hands it draws its picker from. None of them invents anything -- they
       put values where the protocol would not -- so none needs a dev server,
       the same as watching a game again does not. (Reading every hand at once
       does, and says so for itself.) What plays a game on is not offered: a
       copy has a game already. */
    if (m.replay) {
      ...
      if (m.action === 'hands') return devHands(ws, copy, true);
      /* A picture is a look, not a state. It is not in the record -- the device
         that owns one brings it back, and lib/tables.js never writes one down
         -- so putting one on a seat leaves the copy the game it is still a
         copy of. It is answered before the guard below and cuts no tape.
         Nobody else is at a copy, so there is no seat here that is somebody
         else's to dress: this is the one control the copy takes that a table
         would want DEV=1 for. */
      if (m.action === 'avatar') {
        const seat = copy.seats[Number(m.seat)];
        if (!seat) return fail(ws, 'no such seat');
        const bad = setAvatar(seat, m.data);
        if (bad) return fail(ws, bad);
        return said(ws, copy);
      }
      /* What the copy was before the control was pressed. A control that lands
         on the value already there -- the trump it is already in, the seat that
         already deals -- changed nothing, and cutting a game's tape for nothing
         is the worst thing this page could do quietly. */
      const was = JSON.stringify(Tables.record(copy));
      if (m.action === 'state') {
        const bad = devState(copy, m.record);
        if (bad) return fail(ws, bad);
      } else if (m.action === 'patch') {
        devPatch(copy, m.patch || {});
      } else if (m.action === 'seatDo') {
        const bad = devSeatDo(copy, m);
        if (bad) return fail(ws, bad);
      } else {
        return fail(ws, 'a copy takes the record and the players panel, and nothing else');
      }
      if (JSON.stringify(Tables.record(copy)) === was) return said(ws, copy);
      return forked(ws, copy);
    }

    const room = ws.ctx && ws.ctx.room;
    if (!room) return fail(ws, 'open a table first');
    if (ws.ctx.role === 'watch') return fail(ws, 'you are watching this table');
    ...
    switch (m.action) {
      ...
      case 'hands': return devHands(ws, room, false);
