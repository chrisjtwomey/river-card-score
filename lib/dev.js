...
module.exports = ({ DEV, G, createRoom, roomOf, listTables, listGames, endTable, attach, send, fail,
  ...
  function replayGames(room) {
    const live = Trail.read(room.code).length;
  ...
  const sayReplay = (ws, room, copy) => send(ws, Object.assign(
    copy ? Replay.say(copy) : { t: 'replay', code: null, of: room.code },
    replayGames(room)));
  ...
  function devReplay(ws, room, m) {
    ...
    if (m.do === 'open') {
      ...
      const points = m.game ? Trail.readGame(String(m.game)) : Trail.read(room.code);
      if (!points.length) {
        return fail(ws, m.game ? 'nothing was written down about that game'
                               : 'nothing has been written down about this table yet');
      }
      shut();
      const copy = createRoom();
      Replay.open(copy, (points[0] && points[0].c) || room.code, points);
