...
/* Who may read the tables this server is running. Only the machine it runs
   on: a table's 4-character code is the only door it has, and a listing handed
   to every browser on the Wi-Fi would open every table to anybody who could
   reach the page. The device that hosts reads it over the loopback, and nobody
   else can pretend to be there. */
const isLocal = (addr) => /^(::1|::ffff:127\.0\.0\.1|127\.\d+\.\d+\.\d+)$/.test(String(addr || ''));

module.exports = ({ PORT, SCHEME, DEV, ROOT, PUB, pictureOf, readGame, listGames, keptIds,
                   listTables, endTable }) => {
  const MIME = {
  ...
  // Every address a device can use to reach the table. PUBLIC_URL replaces the
  // detected addresses, it does not add to them: behind a proxy or in a
  // container the detected ones are private and no device can reach them.
  ...
  /* A target off this network answers best: the address the machine would use to
     reach the world is the one a device on the same Wi-Fi should be told. But a
     device that IS the hotspot, with no mobile data -- on a plane, say -- has no
  ...
  // Keep it fresh: the address changes when the device joins another network, or
  ...
  /* A player who is here typed or scanned an address that works, and it comes
     in with every request they make. That is worth keeping: on a device that
  ...
  function lanUrls() {
    ...
    // Usually the whole answer, and on a device too. But Termux is refused this
  ...
  // White background and black modules whatever the page theme is, or a device
  ...
  function handler(req, res) {
    ...
    if (url === '/tables.json') {                    // the tables running here, for the device that runs them
      if (!isLocal(req.socket && req.socket.remoteAddress)) {
        res.writeHead(404, { 'content-type': 'text/plain' }).end('not found');
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json', 'cache-control': 'no-cache' });
      // A copy of a table is not a table to go to: it is a game already played,
      // reachable from the dev page that opened it and nowhere else.
      const running = (listTables ? listTables() : []).filter((x) => !x.replay);
      res.end(JSON.stringify({ tables: running }));
      return;
    }

    if (url === '/table/end') {                      // a table taken away, by the machine that runs it
      if (!isLocal(req.socket && req.socket.remoteAddress)) {
        res.writeHead(404, { 'content-type': 'text/plain' }).end('not found');
        return;
      }
      // Asked for, never wandered into: a link followed by mistake, or a page
      // fetching one ahead of time, must not end a game.
      if (req.method !== 'POST') { res.writeHead(405, { 'content-type': 'text/plain' }).end('use POST'); return; }
      const gone = endTable ? endTable(String(query.get('c') || '').toUpperCase().slice(0, 4)) : false;
      res.writeHead(gone ? 200 : 404, { 'content-type': 'application/json' });
      res.end(JSON.stringify({ ended: gone }));
      return;
    }

    if (url === '/games.json') {                     // what the table has on file
      /* And which of them can be watched again. A scorecard outlives its trail
         by the cap they share, so a page that offers to put a game back has to
         be told, one game at a time, whether there is anything to put back. */
      const have = keptIds ? keptIds() : new Set();
      const games = (listGames(query.get('code')) || [])
        .map((g) => Object.assign({}, g, { trail: have.has(g.id) }));
      res.writeHead(200, { 'content-type': 'application/json', 'cache-control': 'no-cache' });
