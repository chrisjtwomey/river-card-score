...
module.exports = ({ DEV, CHAT_KEEP, G, A, send, fail, broadcast, Room, addBot, playCard, bidValue,
  ...
  /* A seat put out of the lobby, by the table host or by the clock. The phone
  ...
  /* And taken off the seat, for a seat that stays where it is: a player put
     out of a game in play. Telling the phone is not enough on its own -- a
  ...
  const MESSAGES = {
    ...
    /* The answer to "still there?". The clock is wound back by any message a
       phone sends, so this one has nothing else to do: it is the message a
       phone has when it has nothing to say. */
    here: {
    ...
    /* Who deals. Before a game it is the seat that deals round one. Once the
       ...
       table that deals on the phones dealt to the seats it dealt to: throw the
    ...
    playout: {
      ...
      run(c, m) {
        ...
        /* The seat named, or the one the table is standing on. Named is how a
           player who has gone home is dealt with: their phone is off, and they
    ...
    /* A player put out of a game already in play. The seat cannot go -- the
       ...
       and the key stops working, which is what stops the phone walking back
    ...
    remove: {
      ...
      run(c, m) {
        ...
        /* Told the way a seat put out of the lobby is told, because it is the
           same thing from the phone's side: there is no coming back to it.
    ...
    /* A seat the table was given, handed back -- whether the player left, or
       ...
       is the whole reason the table has their hand, and the phone that held
       the seat may be long gone. So the seat is opened and they come back to
       it by the name they played under, exactly as any phone that lost its
       seat does. */
    letback: {
    ...
    /* The table is standing on a beat that nothing is left to end: a finished
       ...
       every phone waiting on it. This is the move that timer was going to make
