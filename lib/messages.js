...
/* Every message a socket at a table may send, as a table.
   ...
   read here, once, instead of being written out again in every branch:
     ...
     live    true if this is refused while the table is stopped
...
module.exports = ({ DEV, CHAT_KEEP, G, A, send, fail, broadcast, Room, addBot, playCard, bidValue,
  ...
  const MESSAGES = {
    ...
    playout: {
      ...
      run(c, m) {
        ...
        /* The seat named, or the one the table is standing on. Named is how a
           ...
           behind. Unnamed is the button on the screen that is stopped. */
    ...
    /* Stop the table, and let it go again.

       A stopped table is stopped for everybody: it plays none of its own
       ...
       cards can be stopped too, which is the table most likely to want to
       stop for a moment.

       Everything whoever runs the table does to put a game right goes on
       working while it is stopped: that is what it was stopped for. Said
    ...
    /* The table stopped on a seat nobody was behind, and whoever runs it has
       looked. With real cards there is nothing to hand over -- that player's
    ...
    carryon: {
      ...
      when: (c) => !!c.room.stalled || 'the table is not stopped on anybody',
  ...
  function handleTable(ws, m, c) {
    ...
    /* The table has been stopped. Only what moves the hand on is held: every
       verb that puts a game right still works, because putting it right is
       what a table is stopped for. */
    if (spec.live && c.room.paused) return fail(ws, 'the table is stopped');
    if (spec.when) {
