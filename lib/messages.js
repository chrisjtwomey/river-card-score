...
module.exports = ({ DEV, CHAT_KEEP, G, send, fail, broadcast, seatIndex, curRound, syncCfg,
                    newGame, unfinish, addBot, seatBid, virtual, dealHands, playCard,
                    scoreRound, bumDeal, bidValue, markPresence }) => {
  ...
  const MESSAGES = {
    ...
    playout: {
      who: 'boss', denied: 'only the table host can hand a seat to the table',
      deck: 'virtual', wrongDeck: 'the table has no cards to play that hand with',
      phase: ['bid', 'tricks'], stage: 'no hand in play',
      run(c) {
        const { room, n, r } = c;
        if (!r) return 'no hand in play';
        const p = room.phase === 'bid' ? G.turnSeat(r, n) : (room.play ? room.play.turn : null);
        if (p === null || p === undefined) return 'nobody is on play';
        const seat = room.seats[p];
        if (seat.bot) return `${seat.name} is a player the table provides`;
        if (seat.online) return `${seat.name} is at the table`;
        if (seat.left) return `the table is already playing ${seat.name}'s hand`;
        standDown(room, seat);
      },
    },
    ...
    /* Table talk. It belongs to the table and not to a game, so it lasts as
       ...
       Nothing is written to disk. The talk goes when the server does. */
    chat: {
      run(c, m) {
