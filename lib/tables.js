...
/* A table in play, on disk, so that it outlives the server it is on.
   ...
   is in memory, so the game goes with it. Every other device still holds its
   ...
   and every device keeps its own and hands it over again when it comes back.
...
module.exports = ({ DATA, KEEP_HOURS }) => {
  ...
  const SERVERS_OWN = ['sockets', 'seen', 'trail', 'replay', 'replayTimer', 'botTimer', 'bidTimer'];
  ...
  function record(room) {
    ...
    out.seats = room.seats.map((s) => {
      ...
      delete seat.av;                    // the device that owns it brings it back
      return seat;
    });
    return out;
  }
  ...
  function save(room) {
    // A copy of a table is never read back: it is a game being watched again,
    // and it goes when the page watching it does.
