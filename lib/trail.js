...
/* What happened to a table, written down as it happened.
   ...
   megabytes over one game, on a machine that may be a device. Appending a line
   costs the line.
...
module.exports = ({ DATA, KEEP_HOURS, KEEP_GAMES, TRAIL_MAX, record }) => {
  ...
  /* One thing that happened, held on the room until the server writes it down.
     ...
     A copy of a table being read writes nothing: it is a game being put back,
     not one being played, and its verbs run exactly as the real ones did --
     which is why `reading` is set while it is put back. Without that, seeking
     would write down every point it walked through on the way.

     A copy that has been changed is another thing. It forked where it was
     changed, so what happens on it from there is its own history: it goes onto
     the copy's own points, which is the tape the timeline is drawn from, so
     playing a fork lengthens it and you can scrub back over what you played.
     Never onto disk -- `room.trail` is what `flush` writes, and a copy never
     has one. */
  function point(room, ev) {
    if (!room) return;
    if (room.replay) {
      const r = room.replay;
      if (r.forked && !r.reading) {
        r.points.push(ev);
        // And the copy is standing on what it just did, the way it stands on
        // whatever the transport last put it on. The head follows the play.
